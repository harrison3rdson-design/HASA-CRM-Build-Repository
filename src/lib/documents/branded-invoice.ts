...
export async function generateBrandedInvoicePdf(invoiceId: string) {
  ...
  header img{max-width:280px;max-height:88px;object-fit:contain}
  ...
  <header><div><img src="${logo}" alt="HASA Concepts"></div>
  <div class="invoice-title"><h1>INVOICE</h1><strong>${esc(invoice.invoice_number)}</strong></div></header>
  ...
  <div class="balance"><span>Balance Due</span><span>${money(invoice.balance_due)}</span></div></div>
  ...
  await admin.rpc("attach_invoice_document",{
    p_invoice_id:invoiceId,p_storage_path:path,p_sha256_hash:pdf.sha256
  });
