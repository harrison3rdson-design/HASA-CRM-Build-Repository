create or replace function public.update_proposal_revision_draft_v2(
  p_revision_id uuid,
...
begin
  ...
  select
    ...
    nullif(btrim(section.heading), ''),
    btrim(section.content),
    coalesce(section.sort_order, 0)
  from jsonb_to_recordset(p_sections) as section(
    section_type text,
    heading text,
    content text,
...
revoke execute on function public.update_proposal_revision_draft_v2(
  uuid, text, integer, text, jsonb, jsonb, jsonb
) from public, anon, authenticated;

grant execute on function public.update_proposal_revision_draft_v2(
  uuid, text, integer, text, jsonb, jsonb, jsonb
) to service_role;
